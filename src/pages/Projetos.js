...
function Projetos({ listaProjetos }) { // Define o componente Projetos
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:8080/projetos/nome/${searchTerm}` // Faz uma requisição para buscar projetos
      );
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8080/turmas") // Busca as turmas
      .then((response) => {
    ...
    axios
      .get("http://localhost:8080/categorias") // Busca as categorias
      .then((response) => {
    ...
    const fetchProjetos = async () => {
      try {
        const response = await axios.get("http://localhost:8080/projetos"); // Busca todos os projetos
